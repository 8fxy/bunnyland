...
- 对话适合气泡显示即可，不必太短；中文单句 20 个字以内，包含法语/拉丁字母文本时可放宽到 60 字以内，最长显示 2 ticks。
- 睡觉、打盹、晕倒、熟睡、午睡、躺平等状态不要安排自由活动式动作。
- 0:00-8:00尽量安排睡觉，可以偶尔有兔兔早起出门，或者熬夜看星星等。
- 请适当更新 memory 中的 long_memory.yaml 和 relationships.yaml；每只兔兔最多保留20条重要长期记忆，每只兔兔名下最多保留20条关系提示。达到上限后应替换较旧或重复的内容，而不是继续追加。
- 初夏期间要使用 world_state.yaml 中的荷花、森林、露营、溯溪、赏荷和观星世界事件。
- 世界杯剧情必须区分已确认赛果和待赛日程；只允许根据已确认赛果改变情绪，不得编造尚未结束的比分。
- 世界杯直播事件必须读取 world_state.yaml 的 `fixtures_shanghai`，按 `Asia/Shanghai` 的 `local_date + kickoff` 精确生成；不得仅凭“晚上”随机安排看球。
